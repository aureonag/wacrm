...
/** Contract summary for a kickoff task (one block per signed contract). Deliberately has no price anywhere. */
export function TaskContractCard({ taskId }: { taskId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const data = (await res.json()) as { contracts: ContractInfo[] };
      setState(data.contracts.length > 0 ? data.contracts : "none");
    })();
    return () => {
      cancelled = true;
    };
      ...
      {state.map((c, idx) => (
        <div key={idx} className="space-y-4">
          <div className="rounded-lg border border-border bg-muted/40 p-4">
            {c.title && <p className="mb-1 text-xs font-medium uppercase tracking-wide text-primary">{c.title}</p>}
            ...
            <dl className="mt-2 grid gap-x-6 gap-y-1 text-xs sm:grid-cols-3">
              <div>
                ...
                <dd className="text-foreground">{c.cnpj}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">{t("signedAt")}</dt>
                <dd className="text-foreground">{brDate(c.signedAt)}</dd>
              </div>
              {c.terminatedAt && (
                <div>
                  <dt className="text-red-400">{t("terminated")}</dt>
                  <dd className="text-red-300">{t("endsOn", { date: brDate(c.terminationEffectiveDate) })}</dd>
                </div>
              )}
            </dl>
          </div>
