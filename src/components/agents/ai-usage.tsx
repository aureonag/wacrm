...
import { BarChart3, Bot, PencilLine, Radar } from 'lucide-react';
...
interface UsageResponse {
  ...
  by_mode: {
    ...
    prospecting: { calls: number; tokens: number };
  };
  by_model: {
    model: string;
    provider: string;
        ...
        {loading || !data ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
              ...
              <Stat
                label="Prospecting"
                value={formatCompactNumber(data.by_mode.prospecting.tokens)}
                icon={Radar}
              />
            </div>
