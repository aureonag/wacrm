...
// Same visual chrome as the Comercial/Operational sidebars (deliberately
// a separate component, not a shared one parameterized by environment —
// same reasoning as OperationalSidebar's own comment: the nav lists will
// keep diverging). No permission gating on individual rows here: entry
// to the whole environment is already owner-only (FinanceiroShell),
// which is the entire access model — there's no "owner with some
// modules hidden" case to support, unlike Operational's cargo-based
// per-module gates.
...
export function FinanceiroSidebar({ open = false, onClose }: FinanceiroSidebarProps) {
    ...
    <>
      ...
      >
        <div className="flex h-14 shrink-0 items-center justify-between gap-2 border-b border-border px-6">
          <Link href="/financeiro/dashboard" className="flex items-center gap-2">
            <img
        ...
        <div className="pt-3">
          <EnvironmentSwitcher current="financeiro" />
        </div>
